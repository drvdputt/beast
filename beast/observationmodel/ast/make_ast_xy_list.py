...
def pick_positions_per_mapped_quantity(chosen_seds, q_map, N_bins,
                                       outfile=None, refimage=None, Nrealize=1):
    """
    Spreads a set of fake stars across regions where a certain quantity
    contained in the given map is similar. This quantity can be a
    background luminosity, or a source density, as generated by
    'create_background_density_map' or 'create_source_density_map' in
    the tools directory, respectively.

    The tiles of the given map are divided between a given number of
    ...
    regime of background emission or source density, making it possible
    to have a separate noise model for each of these regions.
    ...
    q_map: str
        Path to a fits file containing a map for the relevant quantity.
        Each row in the fits table should represent a tile of the map.
        The table should have columns describing for each tile: the
        minimum and maximum RA, the minimum and maximum DEC, and a
        value.

    N_bins: int
        The number of bins for the mapped quantity. The bins will be
        picked on a linear grid, ranging from the minimum to the maximum
        value contained in the the map. Then, each tile will be put into
        one of these bins.
    ...
    q = Table.read(q_map)
    tile_vals = q['value']
    min_q = np.amin(tile_vals)
    max_q = np.amax(tile_vals)

    # Create the background bins
    # [min, ., ., ., max]
    q_bins = np.linspace(min_q - 0.01 * abs(min_q),
                         max_q + 0.01 * abs(max_q), N_bins + 1)
    ...
    bin_foreach_tile = np.digitize(tile_vals, q_bins)
    ...
    tile_ra_min = q['min_ra']
    tile_dec_min = q['min_dec']
    tile_ra_delta = q['max_ra'] - tile_ra_min
    tile_dec_delta = q['max_dec'] - tile_dec_min
    ...
    pbar = Pbar(len(tile_sets),
                desc='{} models per spatial region'.format(Nseds_per_region))
